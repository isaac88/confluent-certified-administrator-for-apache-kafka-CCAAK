...
- Each partition is split into segments. By default, each segment contains either 1 GB (`log.segment.bytes`) of data or data from one week|7days (`log.segment.ms`), whichever is smaller.
...
- `log.retention.ms|minutes|hours` -> `retention.ms`
